...
- 13 座學習網站的共用計數元件上方提供「自學星圖」返回入口，方便從任一科目回到總覽。
...
`platform-counter.js` 是 13 座自學平台共用的右下角元件，包含返回「自學星圖」的入口與公開計數器。各站只需傳入固定的 `data-site`，即可讀取 `/api/platform-presence`；資料共用同一個 D1，但依站點識別碼分開統計，不會和自學星圖入口或其他平台混算。
